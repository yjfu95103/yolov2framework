...
class Darknet(nn.Module):
    ...
    def forward(self, x):
        ...
        self.loss = None
        outputs = dict()
        #print("outblock",len(x),len(x[0]),len(x[0][0]),len(x[0][0][0]))
        for block in self.blocks:
            ind = ind + 1
            #print('insideblock',len(x),len(x[0]),len(x[0][0]),len(x[0][0][0]))
            #if ind == 0:
                #print(x)
            #print("x",ind,x)
            #if ind > 0:
            ...
            if block['type'] == 'net':
                ...
                if block['type'] == 'convolutional':
                    ...
                    for i in range(m):
                        minarray = c_array.min()
                        rangeminarray = c_array[0][i].min()
                        maxarray = c_array.max()
                ...
                #     readimg2 = cv2.imread('data/img/conv0_0.jpg')
                #     readimg2 = cv2.cvtColor(readimg,cv2.COLOR_BGR2RGB)
                #     res2 = cv2.resize(readimg2,(416,416),interpolation=cv2.INTER_CUBIC)
                #     r1,g1,b1 = cv2.split(res2)
                    # r1.resize(416,416)
                    # g1.resize(416,416)
                    # b1.resize(416,416)
                    # R.resize(416,416)
                ...
                #         savename = 'data/img/conv' + str(ind) +'_'+str(i)+'.jpg'
                #         data.save(savename)
